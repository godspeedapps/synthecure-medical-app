...
class EditHospitals extends ConsumerStatefulWidget {
  final Doctor model;
  const EditHospitals({super.key, required this.model});

  @override
  EditHospitalsState createState() => EditHospitalsState();
}

class EditHospitalsState extends ConsumerState<EditHospitals> {
  ...
  Widget build(BuildContext context) {
    ref.listen<AsyncValue>(
      adminDoctorUpdateControllerProvider,
      (_, state) {
        state.showAlertDialogUpdate(
            context, message: "Dr.${widget.model.name}'s hospitals have been successfully updated.", title: "Doctor updated!");
      },
    );
                  ...
                  searchResult: Material(
                    child: Consumer(
                      builder: (context, ref, child) {
                        final hospitalStream = ref
                            .watch(filteredHospitalsProvider);
          ...
          Positioned(
            ...
            child: Consumer(
              builder: (context, ref, child) {
                ...
                final state = ref
                    .watch(adminDoctorUpdateControllerProvider);
                        ...
                        ? () async {
                          ...
                          await ref
                              ...
                              .updateDoctorHospitals(
                                  doctor: widget.model, updatedHospitals: hospitals);
...
class HospitalTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    String doctorText = "No Doctors";
    if (model.doctors!.isNotEmpty) {
      final randomDoctor =
          (model.doctors!..shuffle()).first;
      final othersCount = model.doctors!.length - 1;
      doctorText = othersCount > 0
          ? "Dr. ${randomDoctor.name} and $othersCount others"
          : "Dr. ${randomDoctor.name}";
    }
